    ...
    handleDialog(
      `Bạn có chắc chắn muốn xóa ${
        item.type === 'phone'
          ...
          ? 'Laptop'
          : ''
      } ${item.name} của hãng ${item.brand.name} không?`,
      true,
      `Xóa: ${
        item.type === 'phone'
          ? 'Điện thoại'
    ...
    <div className="listItemsAdminTitle d-flex flex-column">
      <div className="p-3">
        <div className="d-flex align-items-center mb-4 qlsp">
          ...
          &nbsp;
          <button
            className="btnDeleteAllItems btn btn-danger"
          ...
          <h2 className="fw-bold totalCountPur">
            (<FontAwesomeIcon icon={faCube}></FontAwesomeIcon> Tất cả:{' '}
            {itemList.length} sản phẩm)
          </h2>
        </div>
            ...
            <tr>
              <th scope="col" style={{ width: '4%' }}>
                <input
                  ...
                  name="allSelect"
                  checked={selectedItem?.length === itemList?.length}
                  onChange={(e) => handleCheckbox(e, itemList)}
                ></input>
              </th>
                ...
                <tr key={item._id} id={item._id}>
                  <td>
                    <input
                      ...
                      className="form-check-input"
                      name={item.name}
                      checked={selectedItem.some((i) => i?._id === item._id)}
      ...
      <div className="divCustomBtn">
        ...
        <input
          ...
          id="gotoPageNum"
          min={1}
          max={ceil(itemListLength / itemsPerPage)}
          defaultValue={1}
        ></input>
        <button
          className="myCustomBtn btn btn-outline-primary"
